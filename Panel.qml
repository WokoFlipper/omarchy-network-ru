...
Panel {
  ...
  // is currently running so it can render "Подключение…" / "Отключение…" /
  // "Забываем…". `passwordSsid` is the row currently expanded into
  ...
  readonly property var dnsProviders: ["DHCP", "Quad9", "DNS4EU", "OpenDNS", "Custom"]
  ...
  function runNetworkAction(kind, network, callback) {
    ...
    // "Подключение…" / "Отключение…" forever.
  ...
  Timer {
    ...
    // while the action is still tracked to show "Неверный пароль" and reopen
    ...
    onTriggered: {
      ...
      var reason
      if (root.actionKind === "connect") reason = "Превышено время подключения"
      else if (root.actionKind === "disconnect") reason = "Превышено время отключения"
      else reason = "Превышено время удаления сети"
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      Item {
        ...
        RowLayout {
          ...
          Button {
            ...
            iconText: "󰓅"
            tooltipText: "Проверить скорость"
            foreground: root.bar.foreground
            fontFamily: root.bar.fontFamily
            iconSize: Style.font.subtitle * 1.5
            horizontalPadding: Style.space(5)
        ...
        Column {
          ...
          Text {
            ...
            readonly property string title: {
              ...
              return root.info.iface || (root.kind === "disconnected" ? "Отключено" : "Нет соединения")
            }
          ...
          Text {
            ...
            text: {
              if (root.info.type === "wifi") {
                ...
                if (root.kind === "disconnected") return "НЕ ПОДКЛЮЧЕНО"
                return ""
              }
      ...
      Column {
        ...
        // "Автоматически" rides on the header line rather than under the pills: it
            ...
            ToggleSwitch {
              ...
              PanelToolTip {
                ...
                text: root.bandPinned
                  ? "Выбирать диапазон автоматически"
                  : "Остаться на " + root.bandLabel(root.bandCurrent)
                fontFamily: root.bar.fontFamily
              }
            }
          }
      ...
      Column {
        ...
        Row {
          ...
          DnsProviderPill {
            provider: "DHCP"
            index: 0
            tooltipText: "Использовать DNS от DHCP"
          ...
          DnsProviderPill {
            provider: "Quad9"
            index: 1
            tooltipText: "Set DNS to Quad9 (9.9.9.9)"
            width: dnsRow.cellWidth
            onClicked: root.setDns(provider)
          }
  ...
  component NetworkRow: CursorSurface {
    ...
    readonly property string statusText: {
      ...
      if (isBusy && root.actionKind === "connect") return "Подключение…"
      if (isBusy && root.actionKind === "disconnect") return "Отключение…"
      if (isBusy && root.actionKind === "forget") return "Забываем…"
      if (isFailed) return root.failureReason || "Не удалось"
    ...
    Item {
      ...
      Column {
        ...
        Text {
          textFormat: Text.PlainText
          text: row.net ? (row.net.ssid || "Скрытая сеть") : ""
          color: root.bar.foreground
          font.family: root.bar.fontFamily
    ...
    Item {
      ...
      TextField {
        ...
        placeholderText: "Логин (пользователь@домен)"
  ...
  component DetailValue: InfoValue {
    ...
    property string tooltipText: "Копировать в буфер обмена"
