...
function bandSectionTitle(selected, current) {
  if (selected !== "auto") return "ДИАПАЗОН WI-FI"

  var label = bandLabel(current)
  if (label === "") return "ДИАПАЗОН WI-FI"

  return "ДИАПАЗОН WI-FI: " + label.toUpperCase()
}

function bandTooltip(band) {
  if (band === "auto") return "Выбирать диапазон автоматически"
  if (!band) return ""
  return "Остаться на " + bandLabel(band)
}
...
function formatPingLatency(ms, hasSamples) {
  ...
  if (!isFinite(value) || value < 0) return "Таймаут"
...
function wifiSectionTitle(wifiNetworks, index) {
  ...
  if (net.known && index === 0) return "ИЗВЕСТНЫЕ СЕТИ"
  if (!net.known && (index === 0 || (networks[index - 1] && networks[index - 1].known))) return "ДРУГИЕ СЕТИ"
  return ""
}
...
function networkFailureReason(reason, needsCredentials, reasons) {
  var r = reasons || {}
  if (needsCredentials && reason === r.NoSecrets) return "Требуется пароль"
  if (needsCredentials && reason === r.WifiAuthTimeout) return "Неверный пароль"
  if (reason === r.WifiNetworkLost) return "Соединение потеряно"
  if (reason === r.WifiClientDisconnected) return "Отключено"
  if (reason === r.WifiClientFailed) return "Не удалось подключиться"
