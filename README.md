...
По умолчанию гранта нет: каждый клик — auth-диалог через polkit-агента.
Грант (`omarchy-dns.sudoers` из пакета) — опция, ставится вручную:
`sudo install -m 0440 omarchy-dns.sudoers /etc/sudoers.d/omarchy-dns`.
Детали и честные риски — в `SECURITY.md`.
